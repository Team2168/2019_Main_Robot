...
public class Lift extends Subsystem {
   ...
   */
  private static AveragePotentiometer liftPosition;
  /**
  ...
  public Lift(){
    ...
    liftPosition=new AveragePotentiometer(RobotMap.LIFT_POSITION_POT_PBOT, RobotMap.LIFT_POT_VOLTAGE_0_PBOT,
 ...
 * @param _liftSpeed is an input used to set the speed
 */
public void driveLift (double _liftSpeed){
 ...
 */
  private boolean isLiftFullyDownHES(){
...
private static boolean isliftMaxPot(){
  boolean _liftMaxPot;
  if (liftPosition.getRawPos()>=RobotMap.LIFT_POT_VOLTAGE_MAX){
    _liftMaxPot=true;
  }
  else{
    _liftMaxPot=false;
...
private static boolean isliftMinPot(){
  boolean _liftMinPot;
  if (liftPosition.getRawPos()<=RobotMap.LIFT_DOWN_MIN_VOLTAGE){
    _liftMinPot=true;
  }
  else{
    _liftMinPot=false;
